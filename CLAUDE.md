...
6. **Two-Sided Market Making** (`core/engine.py:evaluate_two_sided` → `strategies/two_sided.py:TwoSidedManager`) — Per-market spread capture. When spread ≥ `two_sided_min_spread_cents + 2`, posts limit bid and ask each 1¢ inside NBBO. On fill, cancels the opposite side and unwinds.
